...
public class DoctorSetPassword extends JFrame implements ActionListener {
    private JLabel label1, label2, label3, label4;
    private JTextField textField1, textField2, textField3, textField4, textField5;
    private JPasswordField passwordField1;
    private JButton button1, button2;

    public DoctorSetPassword() {
        super("Set Password");
        setVisible(true);
        ...
        if (e.getSource() == button2) {
            ...
            try{
                ...
                while(sc.hasNext())
                {
                    ...
                    String workdays = sc.next();

                    arr1[k] = new Doctor(id,department,name,email,workdays);
                    k++;
                }
                sc.close();
                ...
                for(int i=0; i<number_of_line; i++) {
                    ...
                    if (arr1[i].getId().equals(ID)) {
                        File f = new File("DoctorPassword.txt");
                        if (!f.exists()) {
                            f.createNewFile();
                        }
