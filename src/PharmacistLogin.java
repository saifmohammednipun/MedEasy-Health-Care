...
public class PharmacistLogin extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == button1) {
            ...
            try{
                File file = new File("PharmacistPassword.txt");
                if(!file.exists()) {
                    file.createNewFile();
                }
                ...
                for(int j=0; j<number_of_line; j++ ) {
                    ...
                    if (arr1[j].getId().equals(idReception) && arr1[j].getPassword().equals(passwordReception)) {
                        JOptionPane.showMessageDialog(null, "Pharmacist Login Successful.");
                        setVisible(true);

                        setVisible(false);
                       PharmacistWindow pharmacistWindow = new PharmacistWindow();
                       pharmacistWindow.setVisible(true);
        ...
        if(e.getSource() == button3){
            setVisible(false);
           PharmacistSetPassword pharmacistSetPassword = new PharmacistSetPassword();
           pharmacistSetPassword.setVisible(true);
        }
    }
}